...
const SigninPage = (props: Props) => {
      ...
      httpGet(`${baseAuthUrl}/session/${authorizeResponse.data.auth_key}`, null)
        .then(sessionResponse => {
          if (sessionResponse.status === 200) {
            if (props.isAppSpaceLogin && props.appId) {
              props.cookies.set(
                props.appspace,
  ...
  const redirectToRequestedApp = (authorizeResponse, sessionResponse) => {
    httpGet(`/app/${props.appId}`, {
      ...
      window.location.href = `${appResponse.data.data.redirect}?authKey=${authorizeResponse.data.auth_key}&space=${props.appspace}${appendString}`;
    });
  };
  ...
  const success = (authorizeResponse, sessionResponse) => {
    ...
    if (props.isAppSpaceLogin) {
      props.cookies.set(props.appspace, authorizeResponse.data.auth_key);
